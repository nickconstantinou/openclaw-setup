...
# ── 7j. INSTALL TELEGRAM TOOLS ─────────────────────────────────────────────────
install_telegram_tools() {
    log "Installing Telegram bot tools..."
    
    local tg_script="$ACTUAL_HOME/.openclaw/bin/tg-send-photo.sh"
    
    # Create bin directory
    mkdir -p "$ACTUAL_HOME/.openclaw/bin"
    
    # Create the telegram send script
    cat << 'SCRIPT' > "$tg_script"
#!/usr/bin/env bash
#
# Telegram Bot Media Sender
...
SCRIPT
    ...
    chmod +x "$tg_script"
    chown "$ACTUAL_USER:$ACTUAL_USER" "$tg_script"
    log "Telegram tools installed: $tg_script"
}
