...
profile openclaw-gateway {
  ...
  /tmp/whisper-*/                      rwk,
  /tmp/whisper-/**                     rwk,
  # HuggingFace model cache (faster-whisper downloads model here on first use)
  # HuggingFace model cache — needs mrw:
  #   rw  = download + write model files
  #   m   = mmap model weights into memory (CTranslate2 loads weights via mmap, not read())
  ...
  # /tmp/** needs k so filelock can flock() arbitrary lock files (not just whisper-* pattern)
  /tmp/                                rwk,
  /tmp/**                              rwk,
  /usr/bin/ffprobe                     rix,
